...
# - DeskDrop Articles Service_Name Modeling

# ## shared_articles.csv

# Contains information about the articles shared in the platform. Each article has its sharing date (timestamp), the original url, Service_Name, content in plain text, the article' lang (Portuguese: pt or English: en) and information about the user who shared the article (author).
# 
...
services_df = pd.read_csv('service_list.csv')
#services_df = services_df[services_df['eventType'] == 'CONTENT SHARED']
services_df.head(5)


# ## users_interactions.csv

# Contains logs of user interactions on shared articles. It can be joined to **articles_shared.csv** by **Service_Id** column.
# 
...
interactions_df = pd.read_csv('user_interactions_ghn.csv')
...
users_interactions_count_df = interactions_df.groupby(['User_Id', 'Service_Id']).size().groupby('User_Id').size()
...
interactions_full_df = interactions_from_selected_users_df                     .groupby(['User_Id', 'Service_Id'])['eventStrength'].sum()                     .apply(smooth_user_preference).reset_index()
...
def get_items_interacted(person_id, interactions_df):
    ...
    interacted_items = interactions_df.loc[person_id]['Service_Id']
...
class ModelEvaluator:
    ...
    def get_not_interacted_items_sample(self, person_id, sample_size, seed=42):
        ...
        all_items = set(services_df['Service_Id'])
    ...
    def evaluate_model_for_user(self, model, person_id):
        ...
        if type(interacted_values_testset['Service_Id']) == pd.Series:
            person_interacted_items_testset = set(interacted_values_testset['Service_Id'])
        else:
            person_interacted_items_testset = set([int(interacted_values_testset['Service_Id'])])  
        ...
        for item_id in person_interacted_items_testset:
            ...
            valid_recs_df = person_recs_df[person_recs_df['Service_Id'].isin(items_to_filter_recs)]                    
            valid_recs = valid_recs_df['Service_Id'].values
...
item_popularity_df = interactions_full_df.groupby('Service_Id')['eventStrength'].sum().sort_values(ascending=False).reset_index()
...
class PopularityRecommender:
    ...
    def recommend_items(self, person_id, items_to_ignore=[], topn=10, verbose=False):
        # Recommend the more popular items that the user hasn't seen yet.
        recommendations_df = self.popularity_df[~self.popularity_df['Service_Id'].isin(items_to_ignore)]                                .sort_values('eventStrength', ascending = False)                                .head(topn)
        ...
        if verbose:
            ...
            recommendations_df = recommendations_df.merge(self.items_df, how = 'left', 
                                                          left_on = 'Service_Id', 
                                                          right_on = 'Service_Id')[['eventStrength', 'Service_Id', 'Service_Name',]]


        return recommendations_df
    
popularity_model = PopularityRecommender(item_popularity_df, services_df)
...
# See this [presentation](https://www.slideshare.net/gabrielspmoreira/discovering-users-Service_Names-of-interest-in-recommender-systems-tdc-sp-2016) (from slide 30) for more information on TF-IDF and Cosine similarity.
...
item_ids = services_df['Service_Id'].tolist()
tfidf_matrix = vectorizer.fit_transform(services_df['Service_Name'] )
...
def build_users_profile(person_id, interactions_indexed_df):
    ...
    user_item_profiles = get_item_profiles(interactions_person_df['Service_Id'])
...
def build_users_profiles(): 
    interactions_indexed_df = interactions_full_df[interactions_full_df['Service_Id']                                                    .isin(services_df['Service_Id'])].set_index('User_Id')
    user_profiles = {}
...
class ContentBasedRecommender:
    ...
    def recommend_items(self, person_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        recommendations_df = pd.DataFrame(similar_items_filtered, columns=['Service_Id', 'recStrength'])                                     .head(topn)
        ...
        if verbose:
            ...
            recommendations_df = recommendations_df.merge(self.items_df, how = 'left', 
                                                          left_on = 'Service_Id', 
                                                          right_on = 'Service_Id')[['recStrength', 'Service_Id', 'Service_Name']]


        return recommendations_df
    
content_based_recommender_model = ContentBasedRecommender(services_df)
...
class CFRecommender:
    ...
    def recommend_items(self, person_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        sorted_user_predictions = self.cf_predictions_df[person_id].sort_values(ascending=False)                                     .reset_index().rename(columns={person_id: 'recStrength'})

        # Recommend the highest predicted rating movies that the user hasn't seen yet.
        recommendations_df = sorted_user_predictions[~sorted_user_predictions['Service_Id'].isin(items_to_ignore)]                                .sort_values('recStrength', ascending = False)                                .head(topn)
...
cf_recommender_model = CFRecommender(cf_preds_df, services_df)
...
class HybridRecommender:
    ...
    def recommend_items(self, person_id, items_to_ignore=[], topn=10, verbose=False):
        ...
        cb_recs_df = self.cb_rec_model.recommend_items(person_id, items_to_ignore=items_to_ignore, verbose=verbose,
                                                           topn=1000).rename(columns={'recStrength': 'recStrengthCB'})
        
        #Getting the top-1000 Collaborative filtering recommendations
        cf_recs_df = self.cf_rec_model.recommend_items(person_id, items_to_ignore=items_to_ignore, verbose=verbose, 
        ...
        if verbose:
            ...
            recommendations_df = recommendations_df.merge(self.items_df, how = 'left', 
                                                          left_on = 'Service_Id', 
                                                          right_on = 'Service_Id')[['recStrengthHybrid', 'Service_Id', 'Service_Name',]]


        return recommendations_df
    
hybrid_recommender_model = HybridRecommender(content_based_recommender_model, cf_recommender_model, services_df)
...
def inspect_interactions(person_id, test_set=True):
    ...
    return interactions_df.loc[person_id].merge(services_df, how = 'left', 
                                                      left_on = 'Service_Id', 
                                                      right_on = 'Service_Id') \
...
print hybrid_recommender_model.recommend_items(120111, topn=20, verbose=True)


def recommend( User_Id):
    User_Id=int(User_Id)
    rec=hybrid_recommender_model.recommend_items(User_Id,topn=20,verbose=True)
    rec=rec['Service_Id']
    rec=rec.tolist()
    rec=json.dumps(rec)
    return rec

print recommend(120111)

# # ConclusionGHN APKS (file://ADMIN-PC/Users/admin/Desktop/GHN%20APKS)
...
# Take a look in this [presentation](https://www.slideshare.net/gabrielspmoreira/discovering-users-Service_Names-of-interest-in-recommender-systems-tdc-sp-2016) where I describe a production recommender system, focused on Content-Based Filtering and Service_Name Modeling techniques.
